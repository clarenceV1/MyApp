 ...
 */
public class ContactsLogisticsFragmentController {

    @Inject
    public ContactsLogisticsFragmentController() {
    }
    ...
    public static List<LogisticsRequetDO> getMockData(ContactsRequestDO contactsRequestDO) {
        List<LogisticsRequetDO> result = new ArrayList<>();
        if (contactsRequestDO != null) {
            ...
            if (key.equals(Constant.LOGISTICS_TYPE)) {
                for (int i = 0; i < 2; i++) {
                    LogisticsRequetDO logisticsRequetDO = new LogisticsRequetDO();
                    if (i == 0) {
