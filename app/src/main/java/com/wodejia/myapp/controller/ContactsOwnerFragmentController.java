 ...
 */
public class ContactsOwnerFragmentController {
    @Inject
    ContactsMainManager contactsMainManager;

    @Inject
    public ContactsOwnerFragmentController() {
    }
    ...
    public List<OwnerRequetDO> getMockData(ContactsRequestDO contactsRequestDO) {
        List<OwnerRequetDO> result = new ArrayList<>();
                ...
                for (int x = 1; x < 12; x++) { //哪栋
                    for (int i = 1; i < 12; i++) {// 哪层
                        for (int j = 1; j < 12; j++) { //哪间
                            userId++;
                            OwnerRequetDO userInfoRequetDO = new OwnerRequetDO();
